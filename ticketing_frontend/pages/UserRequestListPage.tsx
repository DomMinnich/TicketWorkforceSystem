...
const UserRequestCard: React.FC<{ request: UserRequest }> = ({ request }) => (
  <Link
    to={`/requests/users/${request.id}`}
    className="block hover:shadow-xl transition-shadow duration-200"
  >
    <Card
      className={`h-full flex flex-col border-l-4 ${
        request.status === "open" ? "border-blue-500" : "border-gray-400"
      }`}
    >
      <div className="flex-grow">
        <h3 className="text-md font-semibold text-gray-800 dark:text-white mb-1">
          {request.fname} {request.lname} - {request.job_title}
        </h3>
        ...
        <div className="space-y-1 text-xs text-gray-600 dark:text-gray-300">
          <p className="flex items-center">
            <UserPlusIcon className="h-4 w-4 mr-1.5" /> Department:{" "}
            {request.department}
          </p>
          <p className="flex items-center">
            <BuildingOfficeIcon className="h-4 w-4 mr-1.5" /> Start Date:{" "}
...
const UserRequestListPage: React.FC = () => {
  ...
  const fetchRequests = useCallback(async () => {
    ...
    setError(null);
    try {
      let fetchedRequests = await requestService.getUserRequests(searchTerm);
      if (statusFilter !== "all") {
      ...
      addNotification(err.message || "Failed to fetch user requests", "error");
    } finally {
      setLoading(false);
    }
    ...
    <div className="container mx-auto px-4 py-6">
      ...
      {!loading && !error && requests.length === 0 && (
        <p className="text-center text-gray-500 dark:text-gray-400 mt-8">
          No new employee requests found.
        </p>
      )}
